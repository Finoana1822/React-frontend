...
const api = axios.create({
  baseURL: `http://localhost:4000/modules`
})
...
const Module= () => {
  ...
  fetch('http://localhost:4000/modules').then((res)=>{
   return res.json()
  }).then((d)=>{
    setDonne(d)
  })
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    ...
    if(errorList.length < 1){
      api.patch("/modules/"+newData.id, newData)
      .then(res => {
  ...
  const handleRowAdd = (newData, resolve) => {
    //validation
    let errorList = []
    if(newData.designation === undefined){
      errorList.push("Please enter first name")
    }
